...
func TestBrowserHarness(t *testing.T) {
	...
	app, err := server.New(context.Background(), server.Config{Issuer: jwks.URL, Audience: "browser-test", Origin: "https://demo.candlea.cn", APIKey: "fixture-only", SyncerURL: upstream.URL, Static: os.DirFS("../../frontend/dist"), Research: os.DirFS("../../../etf-rotation/dca-dashboard")})
	if err != nil {
		t.Fatal(err)
	}
