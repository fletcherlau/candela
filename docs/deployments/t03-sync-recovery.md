...
[桌面截图](../design/sync-recovery/desktop.png) · [手机截图](../design/sync-recovery/mobile.png) · [恢复记录](../design/sync-recovery/recovery.png)
...
需求轴发现取消处理器未注册生产路由（P1），现已修复：生产与验收共用 `Store.Routes`，HTTP/MySQL 和浏览器测试均经过真实 go-zero 路由器；旧三条路由先使取消测试失败，补齐后通过。规范轴发现禁用按钮外观缺失（P3）及重复事务转换（判断性 P3），现已补齐 aria-disabled 样式／aria-busy，并统一事务内取消和中断转换。两轴只读复核通过。修复后全量 syncer race、管理浏览器和构建复验通过。
