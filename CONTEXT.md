...
ETF 同步按当前启用名单或明确指定代码接受持久批次，对象与截止日在接受时固定（北京时间 18:00 前取前一日，之后取当日）。日线和因子分别从各自已存最新日期的次日开始；任一表无历史时从配置起点回填。两个步骤分别保存检查点，日期不必相同；已覆盖截止日的步骤不再取数。按主键 upsert，重复执行不产生重复行。旧 HTTP、收盘报告、机器人相关调用和 one-shot 通过同一批次协调等待结果；调用方断开不取消后台执行。
