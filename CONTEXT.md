...
ETF 同步按当前启用名单或明确指定代码接受持久批次，对象与截止日在接受时固定，截止日为提交当日（北京时间），保留旧 ETF 与收盘报告的当日取数语义。日线和因子分别从各自已存最新日期的次日开始；任一表无历史时从配置起点回填。两个步骤分别保存检查点，日期不必相同；已覆盖截止日的步骤不再取数。按主键 upsert，重复执行不产生重复行。旧 HTTP、收盘报告、机器人相关调用和 one-shot 通过同一批次协调等待结果；调用方断开不取消后台执行。

### Historical Resync（历史重同步）
对选定日线数据的指定起止日期区间重新从数据源获取数据，补齐缺失记录并更新已有记录的维护操作。区间外数据保持不变，与追补最新数据的 Incremental Sync 区分。ETF 通过既有持久批次提交 `historical` 模式，在接受时固定起止闭区间；日线与因子各自从指定起点分段，不依据全表最新日期跳过旧数据。重试保留原模式、范围和阶段检查点；局部历史查询不扩大增量同步证明的完整覆盖范围。
...
### Sync Retry（同步重试）
针对某次批量同步中的失败数据对象重新发起同步的维护操作。成功对象的已写入数据和成功结果保留，不参与这次重试。ETF 重试固定原批次截止日，沿用失败对象的原阶段范围与检查点，关联父批次和原执行；重复点击同一父批次复用其重试批次。

### Sync Recovery（同步恢复）
未取消的 Sync Run 因服务重启或机器中断而停止后，在服务恢复时沿用原数据对象和日期范围自动继续未完成部分的行为。已完成部分保留，中断与恢复记入该次同步的执行记录。中证全指已实现：执行权过期或服务正常停机后，以新执行权从已提交检查点续跑；起点、截止日和已完成分段保持不变。取消意图与分段提交按同一对象事务锁排序，取消后旧执行者不能写入新段。取消、中断和恢复事件与状态变更一并保存；详情读取同一数据库快照。ETF 复用同一执行权、检查点与恢复语义，使用独立对象和任务表，不混入 Index Series；申万随后迁入。

### Syncer（同步服务）
负责维护平台行情数据并承接 Sync Run 的同步服务。每日业务同步由外部 cron 触发，已提交工作的排队、取消及中断恢复属于同步执行职责。

### Four-ETF Rotation Page（四标的轮动页）
面向已了解该策略的持续跟踪者，供其查看每日计算数据、按历史交易日复盘，并观察策略长期历史模拟表现的页面；操作由用户自行判断，教学内容由独立教程承载。历史日期索引仅包含实际发布或采集尝试的留存日，独立于回测观察范围；显式选日无记录时保留所选日期和缺失状态。

### Pre-close Reference（收盘前参考）
某交易日固定的 14:45 价格快照及据此形成的一组策略计算数据，供用户自行判断，并作为与该日收盘后结果比较的基准；不随之后的盘中行情更新，不包含操作建议。由冻结的 Reference Capture 输入和参数整组计算、首次发布后不可覆盖；每日读取只使用已保存结果，原始输入到齐与参考计算发布为不同状态。

### Reference Capture（固定参考采集）

按交易日固定 14:45 目标的持久采集任务，保存首次有效原始行情及当时历史、因子、日历和参数依据；区分计划时点、源完整时间、请求与实际采集时间。独立于可覆盖的旧 Intraday Snapshot。已保存输入在恢复和重复提交时保持不变；错过目标且没有原始输入时明确缺失。原始数据到齐不代表 Pre-close Reference 已计算或发布。
