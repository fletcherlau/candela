...
### Sync Recovery（同步恢复）
未取消的 Sync Run 因服务重启或机器中断而停止后，在服务恢复时沿用原数据对象和日期范围自动继续未完成部分的行为。已完成部分保留，中断与恢复记入该次同步的执行记录。中证全指已实现：执行权过期或服务正常停机后，以新执行权从已提交检查点续跑；起点、截止日和已完成分段保持不变。取消意图与分段提交按同一对象事务锁排序，取消后旧执行者不能写入新段。取消、中断和恢复事件与状态变更一并保存；详情读取同一数据库快照。ETF／申万随后迁入这一语义。
